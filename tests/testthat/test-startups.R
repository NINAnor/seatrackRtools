...
create_test_startup_sheet <- function(n_rows = 5) {
  tibble(
    ...
    starttime_gmt = as.POSIXct(paste0("2025-01-0", seq_len(min(n_rows, 9)), " 10:00:00"), tz = "GMT"),
    ...
    programmed_gmt_time = as.POSIXct(paste0("2025-01-0", seq_len(min(n_rows, 9)), " 10:00:00"), tz = "GMT"),
  ...
  list(
    data = list(
      `ENCOUNTER DATA` = tibble(
        ...
        logger_model_deployed = c("birdTracker5000", "birdTracker3000"),
        colony = c("Colony1", "Colony2")
      ),
...
describe("Startup Path Retrieval", {
  ...
  test_that("get_startup_paths filters tilde files (temporary files)", {
    ...
    expect_true(any(grepl("valid_startup", result)))
...
describe("Startup Helper Functions", {
  test_that("set_master_startup_value updates cell value", {
    df <- create_test_startup_sheet(3)
  ...
  test_that("set_master_startup_value handles date values", {
    df <- create_test_startup_sheet(2)
...
describe("Critical Missing Field Checking", {
  test_that("check_critical_missing returns FALSE when GPS model with deployment date", {
    startup_rows <- tibble()
  ...
  test_that("add_loggers_from_startup skips unreadable Excel files", {
    ...
    master_import <- list(
      data = list(
        ...
        STARTUP_SHUTDOWN = create_test_startup_sheet(1)
      ),
...
describe("Startup Import Edge Cases", {
  test_that("Empty startup file is handled gracefully", {
    empty_startup <- create_test_startup_sheet(0)
  ...
  test_that("Single row startup file is processed correctly", {
    single_startup <- create_test_startup_sheet(1)
  ...
  test_that("Startup with all NA critical fields handled appropriately", {
    startup <- create_test_startup_sheet(1)
  ...
  test_that("Startup with mixed valid and invalid dates", {
    startup <- create_test_startup_sheet(3)
