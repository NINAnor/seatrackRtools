...
prep_SEATRACK_ColonyRepr <- function(species,
                                     colony) {
  spec_col_positions <- seatrackR::getPositions(
    species = species,
    colony = colony
  )
  ...
  spec_col_positions2 <- spec_col_positions %>%
    ...
    mutate(
      ...
      W_Year = case_when(
        month <= 6 ~ paste(Year - 1, Year, sep = "/"), # determine which winter each position relates to
...
#'
#' This is a wrapper function around `prep_SEATRACK_ColonyRepr()` and the analysis pipeline.
